...
namespace SapphTools.Parsers.Lldp {
    public class Tlv {
        private static OuiCollection ouis = new();
        ...
        public Tlv(byte[] tlvBytes) {
            ...
            switch (Type) {
                ...
                case TlvType.SystemCapabilities:
                    RawPayload = tlvBytes[2..];
                    ushort system = BinaryPrimitives.ReadUInt16BigEndian(RawPayload[0..2]);
                    ushort enabled = BinaryPrimitives.ReadUInt16BigEndian(RawPayload[2..]);
                ...
                case TlvType.ManagementAddress:
                    ...
                    uint intNum = BinaryPrimitives.ReadUInt32BigEndian(RawPayload[offset..(offset + 4)]);
                    offset += 4;
        ...
        public static ushort GetLength(byte[] typeAndLength) {
            if (typeAndLength.Length != 2)
                throw new ArgumentOutOfRangeException("Only two bytes may be provided");
            ...
            switch (subtype) {
                ...
                case 5:
                    ...
                    if (RawPayload.Length != 4 && RawPayload.Length != 16)
                        throw new ArgumentOutOfRangeException($"TlvRecord indicates {Type} {Subtype}, Length was {Length} bytes");
                    Data = new System.Net.IPAddress(RawPayload);
                    break;
            }
        }
